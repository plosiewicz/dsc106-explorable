<script>
  ...
  let categories = ['BEEF', 'PORK', 'LAMB', 'CHICKEN', 'TURKEY', 'VEAL', 'CEREALS RTE', 'CEREALS', 'BABYFOOD', 'SOUP', 'CAMPBELL SOUP COMPANY', 'FAST FOODS', "MCDONALD'S", 'BEANS', 'CHEESE', 'CANDIES', 'COOKIES', 'POTATOES', 'ALCOHOLIC BEV', 'OIL'];
  ...
  function drawHeatmap() {
    ...
    const y = d3.scaleBand().domain(nutrients).range([height, 0]).padding(0.05);
    ...
    nutrients.forEach((nutrient) => {
      ...
      const colorScale = d3
        .scaleLinear()
        .domain([
          d3.min(data, (d) => +d[nutrient]),
          nutrientAvg,
          d3.max(data, (d) => +d[nutrient]),
        ])
    ...
    svg
      ...
      .on("mouseover", (event, d) => {
        ...
        const categoryAvgValue = d3.mean(
          data
            .filter((item) => item.Category === d.category)
            .map((item) => +item[d.nutrient]),
        );
        ...
        tooltip
          .style("opacity", 0.9)
          .html(
            `Category: ${d.category}<br>Nutrient: ${d.nutrient}<br>Category Average: ${categoryAvgValue.toFixed(2)}<br>Nutrient Average: ${nutrientAvgValue.toFixed(2)}`,
          );
        tooltip
  ...
  function drawBarChart(topFoods, nutrient, category) {
    ...
    svg.selectAll(".y-axis .tick text").text((d) => {
      const parts = d.split(".");
    ...
    svg
      ...
      .attr("fill", "red")
      .text(
        `Nutritional Content Avg. Per Category: ${categoryAvgValue.toFixed(2)}`,
      );
  }
...
<section>
  <h2><strong>Nutritional Content of Various Food Categories</strong></h2>
  <p>
    This heatmap visualizes the nutritional content of various food categories.
    Hover over each cell to see detailed information about the nutrient content
    and average values. Click on a cell to view the top foods in that category
