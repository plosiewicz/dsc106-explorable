...
function ticks(start, stop, count4) {
  ...
  if (start === stop)
    return [start];
  const reverse3 = stop < start, [i1, i2, inc2] = reverse3 ? tickSpec(stop, start, count4) : tickSpec(start, stop, count4);
  if (!(i2 >= i1))
    return [];
...
function tickStep(start, stop, count4) {
  stop = +stop, start = +start, count4 = +count4;
  const reverse3 = stop < start, inc2 = reverse3 ? tickIncrement(stop, start, count4) : tickIncrement(start, stop, count4);
...
function nodrag_default(view) {
  var root4 = view.document.documentElement, selection4 = select_default3(view).on("dragstart.drag", noevent_default, nonpassivecapture);
...
function yesdrag(view, noclick) {
  var root4 = view.document.documentElement, selection4 = select_default3(view).on("dragstart.drag", null);
  if (noclick) {
...
function drag_default() {
  ...
  function beforestart(that, container2, event, d, identifier, touch) {
    var dispatch = listeners.copy(), p = pointer_default2(touch || event, container2), dx, dy, s2;
...
function append_default2(name) {
  var create = typeof name === "function" ? name : creator_default2(name);
  return this.select(function() {
    return this.appendChild(create.apply(this, arguments));
  });
}
...
function insert_default2(name, before) {
  var create = typeof name === "function" ? name : creator_default2(name), select = before == null ? constantNull2 : typeof before === "function" ? before : selector_default2(before);
  return this.select(function() {
    return this.insertBefore(create.apply(this, arguments), select.apply(this, arguments) || null);
  });
}
...
function dispatchEvent2(node, type2, params) {
  var window = window_default2(node), event = window.CustomEvent;
  if (typeof event === "function") {
    event = new event(type2, params);
  } else {
    event = window.document.createEvent("Event");
...
var e22 = Math.sqrt(2);
function tickSpec2(start, stop, count4) {
  const step = (stop - start) / Math.max(0, count4), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e102 ? 10 : error >= e52 ? 5 : error >= e22 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start * inc2);
  ...
  if (i2 < i1 && 0.5 <= count4 && count4 < 2)
    return tickSpec2(start, stop, count4 * 2);
  return [i1, i2, inc2];
}
function ticks2(start, stop, count4) {
  ...
  if (start === stop)
    return [start];
  const reverse3 = stop < start, [i1, i2, inc2] = reverse3 ? tickSpec2(stop, start, count4) : tickSpec2(start, stop, count4);
  if (!(i2 >= i1))
    return [];
  const n = i2 - i1 + 1, ticks3 = new Array(n);
  if (reverse3) {
...
function nice2(start, stop, count4) {
  let prestep;
  while (true) {
    const step = tickIncrement2(start, stop, count4);
    if (step === prestep || step === 0 || !isFinite(step)) {
      return [start, stop];
    } else if (step > 0) {
      start = Math.floor(start / step) * step;
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    function stitch(line) {
      var start = [line[0][0] + x4, line[0][1] + y4], end = [line[1][0] + x4, line[1][1] + y4], startIndex = index4(start), endIndex = index4(end), f, g;
...
function text_default3(input, init) {
...
function dsvParse(parse) {
  return function(input, init, row) {
    ...
    return text_default3(input, init).then(function(response) {
...
function parser(type2) {
  return (input, init) => text_default3(input, init).then((text) => new DOMParser().parseFromString(text, type2));
}
...
function link_default(links) {
  var id = index3, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count4, bias, random, iterations2 = 1;
  if (links == null)
    links = [];
...
function longitude(point6) {
  return abs3(point6[0]) <= pi2 ? point6[0] : sign(point6[0]) * ((abs3(point6[0]) + pi2) % tau3 - pi2);
}
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m3; ++j, lambda04 = lambda12, sinPhi03 = sinPhi1, cosPhi03 = cosPhi1, point0 = point1) {
      var point1 = ring[j], lambda12 = longitude(point1), phi12 = point1[1] / 2 + quarterPi, sinPhi1 = sin2(phi12), cosPhi1 = cos2(phi12), delta = lambda12 - lambda04, sign3 = delta >= 0 ? 1 : -1, absDelta = sign3 * delta, antimeridian = absDelta > pi2, k2 = sinPhi03 * sinPhi1;
...
function compareIntersection(a4, b) {
  return ((a4 = a4.x)[0] < 0 ? a4[1] - halfPi2 - epsilon4 : halfPi2 - a4[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi2 - epsilon4 : halfPi2 - b[1]);
}
...
function circle_default2(radius) {
  ...
  function intersect2(a4, b, two) {
    ...
    var delta2 = lambda12 - lambda04, polar = abs3(delta2 - pi2) < epsilon4, meridian = polar || delta2 < epsilon4;
    if (!polar && phi12 < phi02)
      z = phi02, phi02 = phi12, phi12 = z;
    if (meridian ? polar ? phi02 + phi12 > 0 ^ q[1] < (abs3(q[0] - lambda04) < epsilon4 ? phi02 : phi12) : phi02 <= q[1] && q[1] <= phi12 : delta2 > pi2 ^ (lambda04 <= q[0] && q[0] <= lambda12)) {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b0, c0, x12, y12, lambda12, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a4 = a0 + a1, b = b0 + b1, c6 = c0 + c1, m3 = sqrt2(a4 * a4 + b * b + c6 * c6), phi2 = asin(c6 /= m3), lambda22 = abs3(abs3(c6) - 1) < epsilon4 || abs3(lambda04 - lambda12) < epsilon4 ? (lambda04 + lambda12) / 2 : atan2(b, a4), p = project(lambda22, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function cluster_default() {
  ...
  function cluster(root4) {
    ...
    var left = leafLeft(root4), right = leafRight(root4), x06 = left.x - separation(left, right) / 2, x12 = right.x + separation(right, left) / 2;
...
function stratify_default() {
  ...
  function stratify(data) {
    var nodes = Array.from(data), currentId = id, currentParentId = parentId, n, d, i, root4, parent, node, nodeId, nodeKey, nodeByKey = /* @__PURE__ */ new Map();
    if (path2 != null) {
...
function treeRoot(root4) {
  var tree = new TreeNode(root4, 0), node, nodes = [tree], child, children4, i, n;
  while (node = nodes.pop()) {
...
function append_default3(name) {
  var create = typeof name === "function" ? name : creator_default3(name);
  return this.select(function() {
    return this.appendChild(create.apply(this, arguments));
  });
}
...
function insert_default3(name, before) {
  var create = typeof name === "function" ? name : creator_default3(name), select = before == null ? constantNull3 : typeof before === "function" ? before : selector_default3(before);
  return this.select(function() {
    return this.insertBefore(create.apply(this, arguments), select.apply(this, arguments) || null);
  });
}
...
function dispatchEvent3(node, type2, params) {
  var window = window_default3(node), event = window.CustomEvent;
  if (typeof event === "function") {
    event = new event(type2, params);
  } else {
    event = window.document.createEvent("Event");
...
function selectAll_default6(selector) {
  return typeof selector === "string" ? new Selection3([document.querySelectorAll(selector)], [document.documentElement]) : new Selection3([array6(selector)], root3);
}
...
function pie_default() {
  var value = identity_default4, sortValues = descending_default, sort3 = null, startAngle = constant_default11(0), endAngle = constant_default11(tau5), padAngle = constant_default11(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum5 = 0, index4 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau5, Math.max(-tau5, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function ticks3(start, stop, count4) {
...
export {
  ...
  selectAll_default6 as selectAll,
